...
def get_word_score(word, n):
    ...
    # first part
    for char in word:
        f_score += SCRABBLE_LETTER_VALUES[char]
    # second part
    s_score = (7*len(word) - 3*(n-len(word)))
    if s_score < 1:
        s_score = 1
...
def is_valid_word(word, hand, word_list):
    ...
    if idx == -1:
        ...
        for v in VOWELS:
            new_word = word
            if new_word.replace('*',v) in word_list:
                return True
        return False
    ...
    while (calculate_handlen(hand) > 0):
        ...
        if user_input == '!!':
            ...
            if is_valid_word(user_input,hand,word_list):
                ...
                print(user_input, 'earned',get_word_score(user_input,calculate_handlen(hand)),'points. Total:',points,'points')
            else:
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    """
    p = 0
    num_hands = int(input("Enter total number of hands: ")) + 1
    HAND_SIZE = int(input("Enter size of hands: "))
    ...
    print("Total score over all hands:",p)
